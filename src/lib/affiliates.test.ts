...
const AMAZON: AffiliateRule = { host: 'amazon.ca', mode: 'param', params: { tag: 'example-tag-21' } };
...
describe('applyAffiliateRules', () => {
  it('Amazon: appends ?tag= to a product URL', () => {
    ...
    expect(out).toBe('https://www.amazon.ca/dp/B0EXAMPLE?tag=example-tag-21');
  });
  ...
  it('Amazon: replaces an existing (scraped) tag with ours', () => {
    ...
    expect(out).toBe('https://amazon.ca/dp/B0EXAMPLE?tag=example-tag-21');
  });
  ...
  it('subdomains match (www., smile., etc.)', () => {
    expect(applyAffiliateRules('https://smile.amazon.ca/x', [AMAZON])).toContain('tag=example-tag-21');
...
describe('schema back-compat', () => {
  ...
  it('old single param/value rules parse as one-entry params', () => {
    const event = fakeEvent(JSON.stringify({
      version: 1,
      rules: [{ host: 'amazon.ca', param: 'tag', value: 'example-tag-21' }],
    }));
    const rules = parseAffiliateRules(event, owner);
    expect(rules).toEqual([{ host: 'amazon.ca', mode: 'param', params: { tag: 'example-tag-21' }, target: undefined }]);
  });
  ...
  it('single-param rules serialize with legacy param/value fields too', () => {
    ...
    expect(written.rules[0].params).toEqual({ tag: 'example-tag-21' });
    expect(written.rules[0].param).toBe('tag');
    expect(written.rules[0].value).toBe('example-tag-21');
  });
